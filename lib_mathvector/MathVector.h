...
#include <initializer_list> // нужно дл€ конструктора от {}
...
public:
    ...
    MathVector(const std::initializer_list<T> list) {
        ...
        size_t i = 0;
        for (const auto& value : list) { //auto автоматически определ€ет тип из list
            this->_data[i] = value;
            this->_states[i] = busy;
            ++i;
        }
    ...
    MathVector<std::common_type_t<T, U>> operator+(const MathVector<U>& other) const {
        using R = std::common_type_t<T, U>; // std::common_type_t<T,U> автоматически выбирает УнаибольшийФ тип
