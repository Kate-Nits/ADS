...
//ѕример работы приложени€:
//#define EASY_EXAMPLE

//ѕриложени€ дл€ работы с окружност€ми/сферами:
//#define POSITION_OF_TWO_CIRCLE
//#define POSITION_OF_TWO_SPHERE
//
//#define USER_INPUT

// ѕриложение дл€ работы с матрицами
//#define WORKING_WITH_MATRIX

//  алькул€тор јрифметических выражений
//#define ARITHMETIC_CALCULATOR

// ѕриложение дл€ лабиринта из DSU и Matrix
...
int main() {
    ...
    Location result = check_position(circle1, circle2);
    set_color(12, 0);
    std::cout << "RESULT:";
    set_color(7, 0);
    print_result_position(" ", result);
#else
    // 1 совпадающие окружности
    Circle circle1(Point(0, 0), 5);
    Circle circle2(Point(0, 0), 5);
    Location result1 = check_position(circle1, circle2);
    print_result_position("1 Coincide circles (same center, same radius)", result1);

    // 2 одина окружность в другой
    Circle circle3(Point(0, 0), 10);
    Circle circle4(Point(1, 1), 2);
    Location result2 = check_position(circle3, circle4);
    print_result_position("2 One circle inside another", result2);

    // 3 внутреннее касание
    Circle circle5(Point(0, 0), 5);
    Circle circle6(Point(3, 0), 2);
    Location result3 = check_position(circle5, circle6);
    print_result_position("3 Internal tangent", result3);

    // 4 пересекающиес€ окружности
    Circle circle7(Point(0, 0), 3);
    Circle circle8(Point(4, 0), 3);
    Location result4 = check_position(circle7, circle8);
    print_result_position("4 Intersecting circles", result4);

    // 5 внешнее касание
    Circle circle9(Point(0, 0), 3);
    Circle circle10(Point(8, 0), 5);
    Location result5 = check_position(circle9, circle10);
    print_result_position("5 External tangent", result5);

    // 6 непересекающиес€ окружности
    Circle circle11(Point(0, 0), 2);
    Circle circle12(Point(10, 0), 3);
    Location result6 = check_position(circle11, circle12);
    print_result_position("6 Non-intersecting circles", result6);
#endif // USER_INPUT
    std::cout << "=======================================" << std::endl;
    return 0;
}
...
int main() {
    ...
    Location result = check_position(sphere1, sphere2);
    set_color(12, 0);
    std::cout << "RESULT:";
    set_color(7, 0);
    print_result_position(" ", result);
    ...
    Location result7 = check_position(sphere1, sphere2);
    print_result_position("1 Coincide spheres (same center, same radius)", result7);

    // 2 одна сфера в другой
    Sphere sphere3(Point3D(0, 0, 0), 8);
    Sphere sphere4(Point3D(1, 1, 1), 2);
    Location result8 = check_position(sphere3, sphere4);
    print_result_position("2 One sphere inside another", result8);

    // 3 внутреннее касание сфер
    Sphere sphere5(Point3D(0, 0, 0), 5);
    Sphere sphere6(Point3D(3, 0, 0), 2);
    Location result9 = check_position(sphere5, sphere6);
    print_result_position("3 Internal tangent spheres", result9);

    // 4 пересекающиес€ сферы
    Sphere sphere7(Point3D(0, 0, 0), 4);
    Sphere sphere8(Point3D(3, 0, 0), 3);
    Location result10 = check_position(sphere7, sphere8);
    print_result_position("4 Intersecting spheres", result10);

    // 5 внешнее касание
    Sphere sphere9(Point3D(0, 0, 0), 3);
    Sphere sphere10(Point3D(0, 8, 0), 5);
    Location result11 = check_position(sphere9, sphere10);
    print_result_position("5 External tangent spheres", result11);

    // 6 не пересекаютс€
    Sphere sphere11(Point3D(0, 0, 0), 2);
    Sphere sphere12(Point3D(0, 0, 15), 3);
    Location result12 = check_position(sphere11, sphere12);
    print_result_position("6 Non-intersecting spheres", result12);
